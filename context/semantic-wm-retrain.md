...
| **C3** | ✅ **BUILT + OFFLINE-VALIDATED 2026-06-11** — token cache (4,500 × [384,16,16] + JPEGs), calibrated τ=0.182 (k=3 chunks), 4,450 temporal + 10,061 shortcut edges → ONE component; wormhole audit clean; 3/3 offline routes (incl. nearhamper, ≥6-episode stitching); runtime = `lekiwi_mpc --graph` (route-progress lookahead — offset-immune; endgame falls back to the real goal image; reach-thresh gated to endgame). Learned φ head still only if drivability gaps appear. **On-robot run pending.** | cache | cross-room goals via waypoint chains |
