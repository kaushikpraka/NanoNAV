...
- **6e — semantic WM retrain (Option C) — ✅ PROTOTYPE ON-ROBOT VALIDATED (2026-06-11): 3/3 arrivals, incl. the goal flat-L2 failed. ✅ C3 SUBGOAL GRAPH BUILT + OFFLINE-VALIDATED (2026-06-11): DIRECTED + direction-certified welds (94.5% SCC), wormhole audit clean, 4/4 offline routes, `lekiwi_mpc --graph` with indexed-waypoint viewer + route strip. ✅ C3 ON-ROBOT SUCCESS (2026-06-12): graph run REACHED nearpurifier (129 steps, 40-hop route, dist 0.08 < thresh) after 3 fixes — calibrated 2τ spacing + raw-progress budget, sticky on-path localization, ≥3-hop waypoint floor; A/B basin bracket 0.35 (in) / 0.45 (out). ⏳ NEXT: verify Pi host connection_time_s edit (NOT a CLI flag — sed the HOST venv's config_lekiwi.py, must print 86400), robot-test the 5-hop sparse subgoals + subgoal-strip viewer (neardesk), collect demo set for write-up (hamper A/B = strongest; purifier #2; neardesk). neardesk endgame hovered ~0.30 → C1 fresh-goal floor question still open; cost-mode-first arm; → then C2 full-room recollect+retrain (published as next chapter of the write-up). Inference-speedup plan (7.3 s→~1 s) parked in plan file. 📝 WRITE-UP (2026-06-13): long-form static site scaffolded under `docs/` (GitHub Pages) — 15-section build-log outline + figure pipeline (`scripts/build_site_assets.py`), prose still to be written; hero demo video pending phone footage. nearhamper graph runs 2026-06-13 made route progress but did not REACH (host blocker recurred). On-robot recordings published as GitHub Release `recordings-v1` (keepers; see [[RECORDINGS]]).** Retrain the WM over frozen
