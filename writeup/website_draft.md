...
*Diffusion Forcing vs standard diffusion. Standard diffusion holds one shared noise level across the whole window and denoises every frame in lockstep from pure noise (k=K) to clean (k=0), so a new frame cannot be produced without denoising the entire sequence again (top). Diffusion Forcing instead assigns every frame its own independent noise level (bottom). At inference the denoising wavefront sweeps forward, keeping past frames clean as context, denoising the next frame, and leaving the future noisy, which is exactly what lets the world model roll itself out one frame at a time and stay stable past the training horizon. Following [Chen et al., 2024](https://arxiv.org/abs/2407.01392).*
...
[FIGURE: ✅ assets/cem_planning.svg]
*The CEM planning loop.*
...
With the graph closing the loop, the planner can finally reach goals well outside the world model's local horizon. These are full runs with the graph in the loop, each routing through a sequence of waypoints from the starting view all the way to the goal.
...
The second run reaches a fan across the room, again routing through the graph from a start with no overlap with the goal.
...
*Note here the initial jerkiness in the trajectory, this is due to non-smooth waypoints generated by the planner. The footage is compressed roughly 88× from the original 12.8-minute run.*
